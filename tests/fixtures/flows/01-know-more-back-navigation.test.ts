/**
 * Generated Test: 01-know-more-back-navigation
 *
 * This test was automatically generated from a recorded USSD session.
 *
 * Session Details:
 * - Flow: 01-know-more-back-navigation
 * - Session ID: rec-01-know-more-back-navigation-1774620295991-2
 * - Phone: +260971230001
 * - Service Code: *2233#
 * - Recorded: 2026-03-27T14:04:59.516Z
 * - Turns: 7
 *
 * Run with:
...
const SESSION_ID = `flow-test-01-know-more-back-navigation-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
...
describe("01-know-more-back-navigation - USSD Flow Test", () => {
  beforeAll(async () => {
  ...
  it('Turn 3: Input: "2"', async () => {
    ...
    // Cumulative USSD text: "1*2"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest("1*2");
  ...
  it('Turn 4: Input: "0"', async () => {
    ...
    // Cumulative USSD text: "1*2*0"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest("1*2*0");
  ...
  it('Turn 5: Input: "3"', async () => {
    // Simulate realistic user interaction timing (2-second delay)
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Cumulative USSD text: "1*2*0*3"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest("1*2*0*3");
  ...
  it('Turn 6: Input: "0"', async () => {
    ...
    // Cumulative USSD text: "1*2*0*3*0"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest("1*2*0*3*0");
  ...
  it('Turn 7: Input: "0"', async () => {
    ...
    // Cumulative USSD text: "1*2*0*3*0*0"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest("1*2*0*3*0*0");
